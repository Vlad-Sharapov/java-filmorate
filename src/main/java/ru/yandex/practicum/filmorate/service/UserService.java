...
public class UserService {
    ...
    public void addFriend(Long id1, Long id2) {
        ...
        if (user1.equals(user2)) {
            throw new ArgumentNotValidException("Пользователь не может добавить себя в друзья.");
        }
    ...
    public void removeFriend(Long id1, Long id2) {
        ...
        User user2 = userStorage.findUser(id2);
        if (user1.equals(user2)) {
            throw new ArgumentNotValidException("Пользователь не может удалить себя из друзей.");
        }
    ...
    public List<User> friends(Long id) {
        ...
        return friendsId.stream().map(userId -> userStorage.getUsers().get(userId)).collect(Collectors.toList());
    }
    ...
    public List<User> commonFriends(Long id1, Long id2) {
        ...
        User user2 = userStorage.findUser(id2);
        if (user1.equals(user2)) {
            throw new ArgumentNotValidException("Пользователь не может посмотреть общий друзей с самим собой.");
        }
        ...
        return user1Friends.stream().filter(user2Friends::contains)
                .map(userId -> userStorage.getUsers().get(userId)).collect(Collectors.toList());
    }
